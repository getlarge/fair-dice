...
- **High risk**: crypto code (`**/crypto*`, `**/random*`, `**/hash*`),
  CI (`.github/`), dependencies (`package-lock.json`, `package.json` dep changes)
- **Medium risk**: new files, config changes, Canvas/UI code (`index.html`),
  build scripts, `.claude/` skills or hooks
- **Low risk**: tests only, documentation, formatting, comments, README
