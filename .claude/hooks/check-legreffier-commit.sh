...
# We don't block, just add context so the agent knows to consider /commit
jq -n '{
  hookSpecificOutput: {
    hookEventName: "PreToolUse",
    additionalContext: "LeGreffier is active. Consider using /commit for medium/high-risk changes (schema, auth, crypto, CI, deps). For low-risk changes (tests, docs, formatting), a normal commit is fine."
  }
}'
